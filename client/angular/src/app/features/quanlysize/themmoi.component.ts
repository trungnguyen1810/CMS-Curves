...
export class ThemMoiSizeComponent implements OnInit {

  @Input() data;
  @Output('CallBack') CallBack = new EventEmitter<boolean>();
  TenSize: string = '';

  constructor(public activeModal: NgbActiveModal, private service: QuanLySizeService, private messageService: MessageService) { }
  ngOnInit(): void {  
  ...
  async saveData() {
    if (!this.TenSize) {
      alert('Tên size bắt buộc nhập'); return;
    }
