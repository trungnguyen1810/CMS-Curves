...
export class ImportKhachHangComponent implements OnInit, OnDestroy {
  @Input() OBJ;
  @Output('CallBack') CallBack = new EventEmitter<boolean>();
  // NguonTin: string = '';
  Title: string = 'Import khách hàng';
  HoVaTen: string = '';
  SoDienThoai: string = '';
  Email: string = '';
  fileimport: string = '';
  ...
  async DownLoad() {
    window.open('/upload/importloi/Fileimportkhachhang.xlsx', "_blank");
  }
  ...
  async Save() {
    ...
    {
      let data = await this.tintucservice.importData( this.datasave.data );
      this.dataluu = data ;
      console.log(data);
